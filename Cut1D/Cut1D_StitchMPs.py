...
def make_StitchMP(num_StitchMPs, dict_primitives, total_time, min_time_modifier, num_rbfs, bandwidth, num_traj):
	...
	bar = Bar('Creating stitchMPs', max = num_StitchMPs)
	...
	for i in range(num_StitchMPs):
		...
		for i, primitive in enumerate(list_primitives):
			...
			for j in range(len(sample) - 1):
				cut_tracker.append(0)
			cut_tracker.append(1)
		...
		# cut_tracker = []
		# for i in range(len(stitchMP)):
		# 	found = False
		# 	index = 0
		# 	while(not found and index < len(stitchMP_cuts)):
		# 		if(stitchMP[i] == stitchMP_cuts[index]):
		# 			found = True
		# 			cut_tracker.append(1)
		# 		index += 1
		# 	if(not found):
